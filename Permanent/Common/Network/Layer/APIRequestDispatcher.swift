...
class APIRequestDispatcher: RequestDispatcherProtocol {
    ...
    #if DEBUG
    /// True when hosting an XCTest **unit** bundle, which must never reach the network: live 401s
    /// log out asynchronously mid-run. UI tests run in a separate process and keep real networking.
    private static let isHostingUnitTests =
        ProcessInfo.processInfo.environment["XCTestConfigurationFilePath"] != nil
    #endif

    /// The default session. `APIRequestDispatcher()` is constructed at hundreds of call sites, so
    /// this is the only practical chokepoint for keeping the unit suite offline.
    ...
    /// As `defaultNetworkSession()`, for the CDN download path — a separate factory because
    /// `CDNSession` has its own configuration, but the offline substitution still happens here.
    ...
    func execute(request: RequestProtocol, createdTask: @escaping (URLSessionTask?) -> Void, completion: @escaping (OperationResult) -> Void) {
        ...
        if !request.skipAuthentication {
            if let token = PermSession.currentSession?.token {
                ...
                // With no Bearer token the server falls back to cookie auth, which needs a CSRF token we don't
                // send — so the request fails. Logged loudly to find when the session token went nil.
    ...
    private func handleJsonTaskResponse(data: Data?, urlResponse: URLResponse?, error: Error?, ignoreErrors: Bool, completion: @escaping (OperationResult) -> Void) {
        // Runs on URLSession's delegate queue, so the `parse` below is off-main and a large listing
        // doesn't block the UI. This shim delivers on main, keeping every caller's guarantee.
        ...
        // Preserve `URLError` so callers can spot a transient connectivity drop and retry without
        // burning an attempt. Reduced to `invalidResponse`, it is indistinguishable from a server bug.
        ...
        case .success(let data):
            ...
            case .success(let json):
                ...
                (message == "warning.auth.mfaToken" && !ignoresMFAWarning && !shouldIgnoreAuthErrors) {
                    ...
                    // The caller's completion must still fire — dropping it hangs spinners and leaks any
                    // continuation bridged to this callback. Delivered as an auth error.
    ...
    private func isNonCriticalEndpoint(_ urlResponse: HTTPURLResponse) -> Bool {
        ...
        // V2 endpoints opt out of the 401 force-logout via `RequestProtocol.ignoreErrors` rather than
        // this URL list: reads are exempt, since foreign-item 401s are expected; writes are not.
        return false
    }
}
