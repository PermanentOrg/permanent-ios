...
// MARK: - Custom TextField with controllable return key
struct FolderNameTextField: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITextField {
        ...
        // Prevent text field from expanding - allow it to be compressed
    ...
    func updateUIView(_ uiView: UITextField, context: Context) {
        // Only update text if it differs to avoid cursor issues
        if uiView.text != text {
            uiView.text = text
        }
        
        // Update coordinator's enabled state
    ...
    class Coordinator: NSObject, UITextFieldDelegate {
        var parent: FolderNameTextField
        weak var textField: UITextField?
        var isReturnKeyEnabled: Bool = false
        
        init(_ parent: FolderNameTextField) {
            self.parent = parent
