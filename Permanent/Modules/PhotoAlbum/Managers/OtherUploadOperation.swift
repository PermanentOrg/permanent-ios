...
class OtherUploadOperation: BaseOperation {
        ...
        apiOperation.execute(in: APIRequestDispatcher()) { [self] result in
            ...
            case .json(let response, _):
                ...
                if model.isSuccessful == true {
                    ...
                    handler(nil)
                    finish()
                    print("Success upload file: \(file.name)")
                    UserDefaults.standard.set(file.creationDate, forKey: "lastCheckedPhotoDate")
                    UserDefaultManager().saveFile(fileInfo: file)
                } else {
                    self.error = UploadError.registerRecord
                    handler(UploadError.registerRecord)
                    finish()
...
extension OtherUploadOperation {
...
extension OtherUploadOperation: URLSessionTaskDelegate {
