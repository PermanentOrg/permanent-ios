                    ...
                    drawerVC.dismiss(animated: false) {
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                            ...
                            if let sharesVC = drawerVC.rootViewController.visibleViewController as? SharesViewController {
                                ...
                                (drawerVC.leftSideMenuController as! SideMenuViewController).selectedMenuOption = TableViewData.drawerData[DrawerSection.leftFiles]![1] //screen shares
        ...
        DispatchQueue.main.async {
            if let drawerVC = self.rootViewController.current as? DrawerViewController {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    ...
                    (drawerVC.leftSideMenuController as! SideMenuViewController).selectedMenuOption = TableViewData.drawerData[DrawerSection.leftOthers]![0] //screen members
                }
            } else {
