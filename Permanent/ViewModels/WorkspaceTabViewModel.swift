...
enum WorkspaceType: Int, CaseIterable {
    ...
    var title: String {
        switch self {
        case .private: return String.private
        case .shared: return String.shared
...
class WorkspaceTabViewModel: ObservableObject {
    ...
    private func updateButtonVisibility() {
        let archivePermissions = currentArchive?.permissions() ?? [.read]
        
        // Show plus button only if user has create AND upload permissions
        let hasCreatePermission = archivePermissions.contains(.create)
        let hasUploadPermission = archivePermissions.contains(.upload)
